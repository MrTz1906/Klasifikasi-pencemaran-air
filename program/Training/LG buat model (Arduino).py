...
arduino_port = 'COM3'  # Ubah dengan port yang sesuai
...
data = []
while True:
    # Baca baris data dari Arduino
    line = ser.readline().decode().strip()

    # Cek jika baris data kosong, maka proses pembacaan selesai
    if line == '':
        break

    # Proses baris data dan tambahkan ke list data
    # Sesuaikan dengan format data yang dikirimkan dari Arduino
    # Contoh: data = line.split(',')
    data.append(line)
...
data_df = pd.DataFrame(data)  # Sesuaikan dengan format data yang dikirimkan dari Arduino

# Ubah nama kolom sesuai dengan kolom pada file CSV Anda
data_df.columns = ['conductivity', 'temperature', 'turbidity', 'total_dissolved_solids']

# Memisahkan fitur input (X) dan variabel target (y)
X = data_df[['conductivity', 'temperature', 'turbidity', 'total_dissolved_solids']]
y = data_df['water_quality_rating']  # Sesuaikan dengan kolom target pada data yang dikirimkan dari Arduino
...
with pd.ExcelWriter(excel_filename) as writer:
    ...
    X_test_with_cm['Klasifikasi'] = ['True Positive' if p == a else 'False Positive' for p, a in
                                     zip(y_pred_mapped, y_test)]
    ...
    confusion_data = pd.DataFrame(
        {'': ['Prediksi Positif', 'Prediksi Negatif'], 'Aktual Positif': [tp, fn], 'Aktual Negatif': [fp, tn]})
